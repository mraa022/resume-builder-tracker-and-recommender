    ...
    if(token){
        jwt.verify(token,process.env.JWT_SECRET,{},async(err,user)=>{
            ...
            for (let i = 0; i < resumes.length; i++){
                ...
                resumesHTML.push({'resume_html':createHeader(currUser)+createEducation(resume)+createWorkExperienceTitle()+createJobs(resume.jobs)+createProjectTitle()+createProjects(resume.projects), 'resume_id':resume._id})
...
const recommendResume = async(req,res)=>{
    const {token} = req.cookies
    const resume_type = req.query.type;
    ...
    if(token){
        jwt.verify(token,process.env.JWT_SECRET,{},async(err,user)=>{
            ...
            const currUser = await User.findOne({email:user.email})
           if(resume_type=='all' || !resume_type){
                all_resumes = await Resume.find({userEmail:user.email})
            }
            else{
                all_resumes = await Resume.find({userEmail:user.email,resumeCategory:resume_type})
            }
            const resumes_text = {}
            for (let i = 0; i < all_resumes.length; i++){
                const resume = all_resumes[i]
                const jobs = []
                const projects = []
                
                for (let j = 0; j < resume.Jobs.length; j++){
                    const job = await findJob(resume.Jobs[j])
                    // push the jobs but flattened to job 
                    jobs.push(`job title is ${job.jobTitle} and company name is ${job.companyName} and job description is ${job.jobResponsibilities.join(' ,')}`)
                }
                for (let j = 0; j < resume.Projects.length; j++){
                    const project = await findProject(resume.Projects[j])
                    projects.push(`project title is ${project.projectTitle} and project subtitle is ${project.projectSubtitle} and project description is ${project.projectDescription.join(' ,')}`)
                }
                resumes_text[resume._id] = `jobs are ${jobs.join(' ,')} and projects are ${projects.join(' ,')}`
            }
            axios.defaults.withCredentials = false
            axios.post('https://resumetracker.pythonanywhere.com/',{resumes:resumes_text,jobDescription}).then(async({data})=>{
                    // get keys of resume_keys sorted by their value
                    const sorted_resume_keys = Object.keys(data).sort((a,b)=>data[b]-data[a])
                    
                    const resumes = []
                    for (let i = 0; i < sorted_resume_keys.length; i++){
                        const resumeHtml = await getOneResumeHtml(sorted_resume_keys[i],currUser)
                        resumes.push(resumeHtml)
                    }
...
const getOneResumeHtml= async(id,currUser)=>{

            const resume = await Resume.findOne({_id: new ObjectId(id)})
            ...
            const modified_resume = result[0]
            
            const final_result = {'resume_html':createHeader(currUser)+createEducation(modified_resume)+createWorkExperienceTitle()+createJobs(modified_resume.jobs)+createProjectTitle()+createProjects(modified_resume.projects), 'resume_id':modified_resume._id}
...
const sortedRecommendedResumes = async(req,res)=>{
    console.log("HEY I GOT HERE")
    const {token} = req.cookies
    const resume_ratings = request.body['resume_ratings']
    // get keys of resume_keys sorted by their value 
    const sorted_resume_keys = Object.keys(resume_ratings).sort((a,b)=>resume_ratings[b]-resume_ratings[a])
    if (token){
        jwt.verify(token,process.env.JWT_SECRET,{},async(err,user)=>{
            if(err) throw err;
            const currUser = await User.findOne({email:user.email})
